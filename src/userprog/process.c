    ...
    //  parse args and store into array
      ...
      // push operation: first move esp and then assign exact info to address
    // stp1: push argv to stack

      int offset = 0;

      for(int cnt = argc-1;cnt>=0;cnt--){
        offset += sizeof(argv[cnt]);
        *esp -= sizeof(argv[cnt]);
        ptrs[cnt] = *esp;
        memcpy(*esp, argv[cnt], sizeof(argv[cnt]));
      }
    //  stp2: push word align
      *esp -= offset%4;
      uint8_t wordalign = 0;
      memcpy(*esp, &wordalign, sizeof(offset%4));
    ...
    //  stp3: push argv addr to stack
      *esp -= sizeof(char*);
      ...
      for(int cnt = argc-1; cnt>=0;cnt--){
        *esp -= sizeof(ptrs[cnt]);
        memcpy(*esp, ptrs[cnt], sizeof(ptrs[cnt]));
      }
    // stp4: push argv[] and argc and return address
      *esp -= sizeof(char**);
      memcpy(*esp, ptrs, sizeof(char**));
      *esp -= sizeof(int);
